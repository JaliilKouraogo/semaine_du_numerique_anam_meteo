...
// Types locaux
interface BulletinExtended extends Omit<BulletinSummary, 'id'> {
  id: string;
...
export function ExplorationBulletinsPage() {
    ...
    const loadBulletins = async () => {
      try {
        ...
        const normalized = items.map((item, index) => ({
          ...item,
          id: item.id?.toString() || `${item.date}-${item.type}-${index}`,
        }));
        setBulletins(normalized);

        // Par défaut, on affiche tous les bulletins (pas de filtre de date)
        setSelectedDate("");
  ...
  const loadMetrics = useCallback(async (date: string) => {
    try {
      ...
      // Metrics are optional (only if forecast exists). 
      // Silently fail if not found to avoid console noise.
      setMetrics(null);
    } finally {
      ...
      const pollStatus = async () => {
        try {
          ...
          if (status.status === "completed") {
            ...
            const results = status.result?.translations;
            if (results && selectedBulletin) {
              const updated = {
                ...selectedBulletin,
                interpretation_francais: results.fr || selectedBulletin.interpretation_francais,
                interpretation_moore: results.moore || selectedBulletin.interpretation_moore,
                interpretation_dioula: results.dioula || selectedBulletin.interpretation_dioula,
              };

              // Mettre à jour l'état local immédiatement
              setSelectedBulletin(updated);
              setBulletins((current) =>
                current.map((b) => (b.id === selectedBulletin.id ? updated : b))
              );
            } else {
              // Fallback au rechargement complet si pas de result direct
              await loadBulletinDetails(selectedBulletin);
            }
...
🔔 Vous pourrez naviguer librement dans l'application pendant le traitement.`,
  ...
  const stats = useMemo(() => {
    ...
    const totalStationsCount = bulletins.reduce((sum, item) => sum + (item.stations_count ?? 0), 0);
  ...
  return (
    <Layout title="Bulletins météorologiques">
      <div className="space-y-6">
        ...
        {toast && (
          <div
            className={`fixed top-4 right-4 z-50 px-6 py-4 rounded-2xl shadow-lg border-l-4 animate-slide-in-right ${toast.type === "success"
          ...
          <div className="grid gap-6 lg:grid-cols-[2fr,1fr]">
                  ...
                  <div className="flex items-center rounded-full border border-[var(--border)] bg-[var(--surface)]/70 p-1">
                    {[
                      ...
                      <button
                        ...
                        className={`flex-1 rounded-full px-3 py-2 text-xs font-semibold transition-all ${filterType === option.key
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="group rounded-2xl border border-[var(--border)] bg-gradient-to-br from-blue-50 to-white p-4 hover:shadow-lg transition-all duration-300 cursor-pointer">
                ...
                <p className="text-xs text-muted">📍 {stats.totalStationsCount} stations totales</p>
              </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className={`rounded-full px-4 py-2 text-sm font-semibold transition-colors flex items-center gap-2 ${viewMode === "list"
          ...
          {selectedBulletin && (
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm font-semibold transition-colors flex items-center gap-2 ${viewMode === "detail"
                ...
                <tbody className="divide-y divide-[var(--border)]">
                  ...
                  {filteredBulletins.map((bulletin) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <div className="max-w-[200px] truncate text-xs text-muted" title={bulletin.title || ""}>
                          {bulletin.title || "---"}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold ${bulletin.type === "observation"
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2">
                          <button
                            ...
                            className="inline-flex items-center gap-2 rounded-full bg-emerald-600 px-4 py-2 text-xs font-semibold text-white hover:bg-emerald-700 transition-colors shadow-sm"
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <p className="text-sm text-muted mt-1">
                        {selectedBulletin.stations?.length || selectedBulletin.stations_count || 0} station(s)
                      </p>
                    </div>
                ...
                <div className="grid gap-4">
                  {LANGUAGE_OPTIONS.filter((lang) => selectedLanguages.includes(lang.key)).map(
                    (language) => {
                      ...
                      const isGenerating =
                        regeneratingLanguage === language.key ||
                        (regeneratingLanguage === "interpretation_francais" && (language.key === "interpretation_moore" || language.key === "interpretation_dioula"));
                      ...
                      return (
                        <div key={language.key} className={panelClasses}>
                          ...
                          <p
                            className={`text-sm leading-relaxed ${content ? "text-ink whitespace-pre-wrap" : "text-muted italic"
                              }`}
                          >
                        ...
                        >
                          ...
                          <div className="grid gap-3 sm:grid-cols-2">
                            ...
                            <div className="rounded-xl bg-blue-50/30 border border-blue-100 p-3">
                              ...
                              {/* Check tmin/tmax raw or obs specific */}
                              <p className="text-sm font-mono text-ink">
                                Tmin {station.tmin_obs ?? (selectedBulletin.type === "observation" ? (station as any).tmin : null) ?? "--"}°C
                              </p>
                              <p className="text-sm font-mono text-ink">
                                Tmax {station.tmax_obs ?? (selectedBulletin.type === "observation" ? (station as any).tmax : null) ?? "--"}°C
                              </p>
                              <p className="text-xs text-muted mt-1">
                                {station.weather_obs ?? (selectedBulletin.type === "observation" ? (station as any).weather_condition : null) ?? "--"}
                              </p>
                            </div>
                            ...
                            <div className="rounded-xl bg-emerald-50/30 border border-emerald-100 p-3">
                              ...
                              <p className="text-sm font-mono text-ink">
                                Tmin {station.tmin_prev ?? (selectedBulletin.type === "forecast" ? (station as any).tmin : null) ?? "--"}°C
                              </p>
                              <p className="text-sm font-mono text-ink">
                                Tmax {station.tmax_prev ?? (selectedBulletin.type === "forecast" ? (station as any).tmax : null) ?? "--"}°C
                              </p>
                              <p className="text-xs text-muted mt-1">
                                {station.weather_prev ?? (selectedBulletin.type === "forecast" ? (station as any).weather_condition : null) ?? "--"}
                              </p>
                            </div>
                          </div>
