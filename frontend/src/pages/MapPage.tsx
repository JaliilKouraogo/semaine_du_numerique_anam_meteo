    ...
    const loadStations = async () => {
      ...
      try {
        ...
        setBulletinInterpretations({
          francais: data.interpretation_francais,
        ...
        data.stations.forEach((station) => {
          ...
          const merged: Station = {
            ...
            interpretation_francais: station.interpretation_francais ?? null,
            interpretation_moore: station.interpretation_moore ?? null,
            interpretation_dioula: station.interpretation_dioula ?? null,
          };
        ...
        <section className="flex flex-1 flex-col gap-3 p-2 min-h-0">
          ...
          <div className="flex flex-wrap gap-2">
            {CHIPS.map((chip) => (
              <button
                key={chip}
                className={`flex h-9 items-center rounded-full px-4 text-sm font-medium ${selectedStations.includes(chip)
            ...
            <aside className="surface-panel p-3 space-y-3 overflow-y-auto max-h-full">
              ...
              {selectedStationData ? (
                <div className="space-y-3 text-sm">
                  ...
                  {/* Station-specific Interpretation if available */}
                  {(selectedStationData.interpretation_francais || selectedStationData.interpretation_moore || selectedStationData.interpretation_dioula) && (
                    <div className="border-t border-[var(--border)] pt-4 mt-2 space-y-3">
                      <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-primary-600">Interprétation Locale</h3>
                      {selectedStationData.interpretation_francais && (
                        <div className="surface-panel p-3 border-l-2 border-emerald-500 bg-emerald-50/5">
                          <p className="text-[9px] font-black uppercase text-emerald-700 mb-1">Français</p>
                          <p className="text-xs text-ink leading-relaxed">{selectedStationData.interpretation_francais}</p>
                        </div>
                      )}
                      {selectedStationData.interpretation_moore && (
                        <div className="surface-panel p-3 border-l-2 border-blue-500 bg-blue-50/5">
                          <p className="text-[9px] font-black uppercase text-blue-700 mb-1">Mòoré</p>
                          <p className="text-xs text-ink leading-relaxed italic text-justify">{selectedStationData.interpretation_moore}</p>
                        </div>
                      )}
                      {selectedStationData.interpretation_dioula && (
                        <div className="surface-panel p-3 border-l-2 border-amber-500 bg-amber-50/5">
                          <p className="text-[9px] font-black uppercase text-amber-700 mb-1">Dioula</p>
                          <p className="text-xs text-ink leading-relaxed italic text-justify">{selectedStationData.interpretation_dioula}</p>
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  <div className="border-t border-[var(--border)] pt-4 mt-4 space-y-3">
                    <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-muted">Bulletin National</h3>
                    {bulletinInterpretations.francais && (
                      <div className="surface-panel p-3 border-l-2 border-emerald-500 bg-emerald-50/5 cursor-default group">
                        <div className="flex items-center justify-between mb-1">
                          <p className="text-[9px] font-black uppercase text-emerald-700">Français</p>
                          <span className="material-symbols-outlined text-[10px] text-muted opacity-0 group-hover:opacity-100 transition-opacity">info</span>
                        </div>
                        <p className="text-[11px] text-ink line-clamp-2 hover:line-clamp-none transition-all duration-300">{bulletinInterpretations.francais}</p>
                      </div>
                    )}
                    {bulletinInterpretations.moore && (
                      <div className="surface-panel p-3 border-l-2 border-blue-500 bg-blue-50/5 cursor-default group">
                        <div className="flex items-center justify-between mb-1">
                          <p className="text-[9px] font-black uppercase text-blue-700">Mòoré</p>
                          <span className="material-symbols-outlined text-[10px] text-muted opacity-0 group-hover:opacity-100 transition-opacity">info</span>
                        </div>
                        <p className="text-[11px] text-ink italic line-clamp-2 hover:line-clamp-none transition-all duration-300">{bulletinInterpretations.moore}</p>
                      </div>
                    )}
                    {bulletinInterpretations.dioula && (
                      <div className="surface-panel p-3 border-l-2 border-amber-500 bg-amber-50/5 cursor-default group">
                        <div className="flex items-center justify-between mb-1">
                          <p className="text-[9px] font-black uppercase text-amber-700">Dioula</p>
                          <span className="material-symbols-outlined text-[10px] text-muted opacity-0 group-hover:opacity-100 transition-opacity">info</span>
                        </div>
                        <p className="text-[11px] text-ink italic line-clamp-2 hover:line-clamp-none transition-all duration-300">{bulletinInterpretations.dioula}</p>
                      </div>
                    )}
                  </div>
                </div>
