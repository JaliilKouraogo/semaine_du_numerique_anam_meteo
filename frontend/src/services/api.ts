  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (isApiErrorPayload(payload)) {
        ...
        // Don't report 404s globally/toasts, let caller handle it
        if (status !== 404) {
          reportError(message);
        }
...
export interface MetricsResponse {
  ...
  observation_file_path?: string | null;
  forecast_file_path?: string | null;
...
export async function uploadBulletin(file: File, options?: { async?: boolean }) {
  const formData = new FormData();
  ...
  try {
    const response = await fetch(`${API_BASE}/upload-bulletin${query}`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${getAuthToken()}`,
      },
    ...
    if (!response.ok) {
      const message =
        (payload as { detail?: string | { message?: string } })?.detail
          ? (typeof (payload as any).detail === 'string' ? (payload as any).detail : (payload as any).detail.message)
    ...
    return payload; // Returns UploadResponse | UploadJobResponse
  } finally {
    finishRequest();
  }
}

export async function fetchUploadJobStatus(jobId: string) {
  return requestJson<UploadJobStatus>(`/upload-bulletin/jobs/${encodeURIComponent(jobId)}`);
}

export async function retryUploadJob(jobId: string) {
  return postJson<UploadJobStatus>(`/upload-bulletin/jobs/${encodeURIComponent(jobId)}/retry`, {});
}

export async function submitFeedback(jobId: string, corrections: any[]) {
  return postJson<{ status: string, message: string }>(`/upload-bulletin/feedback`, {
    job_id: jobId,
...
export async function fetchUploadJobs(limit = 50, type?: string) {
  const query = type ? `?job_type=${encodeURIComponent(type)}&limit=${limit}` : `?limit=${limit}`;
  return requestJson<UploadJobStatus[]>(`/upload-bulletins/jobs${query}`);
}

export async function deleteUploadJob(jobId: string) {
  return requestJson<{ status: string; job_id: string }>(`/upload-bulletins/jobs/${encodeURIComponent(jobId)}`, {
    method: "DELETE",
  });
}
...
export async function fetchJsonMetricsFiles() {
  return requestJson<{ files: JsonMetricsFileInfo[] }>("/json-metrics/files");
}

export async function fetchJsonMetricsFile(path: string) {
  return requestJson<JsonMetricsFilePayload>(`/json-metrics/file?path=${encodeURIComponent(path)}`);
}
