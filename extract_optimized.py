"""
extract_optimized.py - Extraction OPTIMISÉE des températures et icônes météo

Optimisations implémentées :
1. BATCH PROCESSING  : Envoie 1 image complète au lieu de 20 crops
2. PARALLELISATION   : Traite plusieurs cartes en parallèle (ThreadPool)
3. CACHE PAR VILLE   : Sauvegarde après chaque carte, reprend où on s'est arrêté
4. EXTRACTION COMBINÉE: Températures + Icônes en UN seul appel

Gain estimé : 30 min → 2-5 min par bulletin !

Usage:
    python extract_optimized.py
    python extract_optimized.py --workers 4 --model minicpm-v:8b
"""
...
OUTPUT_ROOT = Path("2024_temps_qwen")
CACHE_DIR = Path("2024_temps_qwen/.cache")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Extraction OPTIMISÉE des données météo")
    ...
    print("=" * 70)
    print("🌤️  EXTRACTEUR MÉTÉO OPTIMISÉ")
    print("=" * 70)
    ...
    print(f"\n📍 {len(cities_rel)} villes chargées")
